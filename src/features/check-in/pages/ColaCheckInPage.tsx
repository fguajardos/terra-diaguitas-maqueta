...
export function ColaCheckInPage() {
  ...
  const { atrasadas, hoy } = useMemo(() => {
    ...
    const reservasParaCheckIn = reservas.filter(
      (r) =>
        (r.estado === 'confirmada' || r.estado === 'pagada') && r.fechaIngreso <= hoyISO_
    )

    return {
      atrasadas: reservasParaCheckIn.filter((r) => r.fechaIngreso < hoyISO_),
      hoy: reservasParaCheckIn.filter((r) => r.fechaIngreso === hoyISO_),
    }
  }, [reservas])
              ...
              <Card key={reserva.id} className="p-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Habitación {reserva.habitacionId.split('-').pop()} • Código: {reserva.codigo}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      size="sm"
                      onClick={() => navigate(`/check-in/${reserva.id}`)}
                    >
                      Iniciar
                    </Button>
                  </div>
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Cola de Check-in</h1>
      ...
      {atrasadas.length === 0 && hoy.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-muted-foreground text-lg">No hay reservas pendientes de check-in</p>
        </Card>
      ) : (
        <>
